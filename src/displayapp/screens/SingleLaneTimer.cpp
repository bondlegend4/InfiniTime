...
namespace {
  STimeSeparated_t convertTicksToTimeSegments(const TickType_t timeElapsed) {
    // Centiseconds
    ...
    return STimeSeparated_t {mins, secs, hundredths};
  }

  void play_pause_event_handler(lv_obj_t* obj, lv_event_t event) {
    auto* singleLaneTimer = static_cast<SingleLaneTimer*>(obj->user_data);
    singleLaneTimer->playPauseBtnEventHandler(event);
  }

  void stop_lap_event_handler(lv_obj_t* obj, lv_event_t event) {
    auto* singleLaneTimer = static_cast<SingleLaneTimer*>(obj->user_data);
    singleLaneTimer->stopLapBtnEventHandler(event);
  }
}

SingleLaneTimer::SingleLaneTimer(DisplayApp* app, System::SystemTask& systemTask) : Screen(app), systemTask {systemTask} {
...
SingleLaneTimer::~SingleLaneTimer() {
  lv_task_del(taskRefresh);
  systemTask.PushMessage(Pinetime::System::Messages::EnableSleeping);
  lv_obj_clean(lv_scr_act());
}

void SingleLaneTimer::Reset() {
  currentState = SStates::Init;
  oldTimeElapsed = 0;
  lv_obj_set_style_local_text_color(time, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, Colors::lightGray);
  lv_obj_set_style_local_text_color(msecTime, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, Colors::lightGray);
...
void SingleLaneTimer::Start() {
  ...
  currentState = SStates::Running;
  systemTask.PushMessage(Pinetime::System::Messages::DisableSleeping);
}

void SingleLaneTimer::Pause() {
  startTime = 0;
  // Store the current time elapsed in cache
  oldTimeElapsed = laps[lapsDone];
  currentState = SStates::Halted;
  lv_label_set_text_static(txtPlayPause, Symbols::play);
  lv_label_set_text_static(txtStopLap, Symbols::stop);
...
void SingleLaneTimer::Refresh() {
  if (currentState == SStates::Running) {
    laps[lapsDone] = oldTimeElapsed + xTaskGetTickCount() - startTime;

    STimeSeparated_t currentTimeSeparated = convertTicksToTimeSegments(laps[lapsDone]);
    lv_label_set_text_fmt(time, "%02d:%02d", currentTimeSeparated.mins, currentTimeSeparated.secs);
    lv_label_set_text_fmt(msecTime, "%02d", currentTimeSeparated.hundredths);
  }
}

void SingleLaneTimer::playPauseBtnEventHandler(lv_event_t event) {
  if (event != LV_EVENT_CLICKED) {
    return;
  }
  if (currentState == SStates::Init) {
    Start();
  } else if (currentState == SStates::Running) {
    Pause();
  } else if (currentState == SStates::Halted) {
    Start();
  }
}

void SingleLaneTimer::stopLapBtnEventHandler(lv_event_t event) {
  if (event != LV_EVENT_CLICKED) {
    return;
  }
  ...
  if (currentState == SStates::Running) {
    ...
    for (int i = lapsDone - displayedLaps; i < lapsDone; i++) {
      ...
      STimeSeparated_t times = convertTicksToTimeSegments(laps[i]);
      char buffer[16];
...
bool SingleLaneTimer::OnButtonPushed() {
  if (currentState == SStates::Running) {
    Pause();
    return true;
  }
