...
namespace {
  void lv_update_task(struct _lv_task_t* task) {
    auto* user_data = static_cast<SingleLane*>(task->user_data);
  ...
  void event_handler(lv_obj_t* obj, lv_event_t event) {
    ...
    SingleLane* screen = static_cast<SingleLane*>(obj->user_data);
...
SingleLane::SingleLane(uint8_t screenID,
           uint8_t numScreens,
           DisplayApp* app,
  ...
  for (uint8_t i = 0; i < 6; i++) {
    //TODO colors need to change for each button
...
void SingleLane::UpdateScreen() {
  lv_label_set_text(label_time, dateTimeController.FormattedTime().c_str());
  statusIcons.Update();
}

void SingleLane::OnValueChangedEvent(lv_obj_t* obj, uint32_t buttonId) {
  if (obj != btnm1) {
    return;
  }
  //TODO SingleLane controller must be modified before app is opened up
  //TODO Reset SingleLane when a timer is reached
  app->StartApp(Apps::SingleLane, DisplayApp::FullRefreshDirections::Up);
  running = false;
}
